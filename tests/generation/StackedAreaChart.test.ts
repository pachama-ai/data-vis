...
describe('StackedAreaChart', () => {
  ...
  it('zeigt feste Guide-Linie bei ausgew�hlter Annotation', async () => {
    const wrapper = mount(StackedAreaChart)
    await flushPromises()
  ...
  it('zeigt Default-Sidebar mit �bersicht', async () => {
    const wrapper = mount(StackedAreaChart)
    await flushPromises()
  ...
  it('zeigt Quellenzustand bei Legendenklick', async () => {
    ...
    expect(wrapper.text()).toContain('H�chster Monatswert')
    expect(wrapper.text()).toContain('Niedrigster Monatswert')
  })

  it('zeigt wieder �bersicht nach erneutem Legendenklick', async () => {
    const wrapper = mount(StackedAreaChart)
    await flushPromises()
  ...
  it('zeigt Annotation-Zustand bei Marker-Klick', async () => {
    ...
    expect(wrapper.text()).toContain('Anteile im ausgew�hlten Monat')
  })
  ...
  it('"Alle"-Button setzt Hervorhebung zur�ck', async () => {
    const wrapper = mount(StackedAreaChart)
    await flushPromises()

    // Photovoltaik ausw�hlen
    const pvButton = wrapper.findAll('button').find((button) => {
      return button.text().includes('Photovoltaik')
    })!
    ...
    // Pr�fe: pv-Layer opacity 1, gas-Layer opacity 0.15
    expect(
    ...
    // Pr�fe: alle Layer wieder opacity 1
    expect(
      wrapper.find('.layer-pv').attributes('opacity'),
    ).toBe('1')
  ...
  it('Marker-Bereich enth�lt nicht "Aktueller Datenstand"', async () => {
    const wrapper = mount(StackedAreaChart)
    await flushPromises()
